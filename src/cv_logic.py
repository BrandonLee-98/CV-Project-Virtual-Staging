...
def call_replicate_seedream(
    ...
    When ``USE_LIVE_API`` is True
        Uploads the image to Replicate, calls model
        ``bytedance/seedream-4.5`` with the same parameters the
        production app uses, downloads the resulting image, and writes
        it to ``out_path``. Requires ``REPLICATE_API_TOKEN`` in the
        environment and the ``replicate`` package installed.

    When ``USE_LIVE_API`` is False
        Skips the network entirely and copies
        ``results/placeholder_output.jpg`` to ``out_path``. Useful for
        iterating on prompts without burning API credits.

    Parameters
    ----------
    image_path
        Path to the *prepared* image (call ``prepare_image_for_staging``
        first). Replicate accepts a public URL or a local file handle.
    prompt
        Full staging prompt. Build with ``build_staging_prompt(...)``.
    out_path
        Where to write the resulting staged image.
    size
        Output dimensions to request from SeeDream. Defaults to the
        production "1024x1024".
    ...
    if isinstance(output, str):
        ...
        # Mirror TS defensive parsing at server/controlnet.ts:219-226 —
        # SeeDream sometimes returns {"data": [{"url": "..."}]}.
